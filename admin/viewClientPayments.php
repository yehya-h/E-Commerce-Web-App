...
<head>
    ...
    <title>View Payments Information</title>
</head>
...
<body>
    ...
    if (isset ($_GET['client_id'])) {  //view accounts --> specific account info
        $payments = $dbHelper->getPaymentInfoByClientId($_GET['client_id']);
        if (is_array($payments) && $payments != null) {
            echo "<h3>Choose a payment info to display</h3>
            <table border=1>
            <tr><th>Payment ID</th>
            <th>Payment Info</th>
            </tr>";
            foreach ($payments as $value) {
                echo "<tr><td>" . $value->getPaymentInfoId() . "</td>
                <td><button class='i-button' onclick='loadData(" . $value->getPaymentInfoId() . ")'>More Info</button></td></tr>";
            }
    ...
    <script>
        function loadData(payment_id) {
            var url = 'showPayment_client.php?payment_id=' + encodeURIComponent(payment_id);
